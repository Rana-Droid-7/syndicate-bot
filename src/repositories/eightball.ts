...
export const eightBallRepository = {
  add(content: string, createdBy: string): number {
    ...
    return Number(
      stmt(`INSERT INTO eightball (content, created_by) VALUES (?, ?)`).run(
        content,
        createdBy,
      ).lastInsertRowid,
    );
  ...
  randomWithUsage(): EightBallRow | null {
    ...
    const run = database.transaction(() => {
      const row = database
        .prepare(
          `SELECT * FROM eightball WHERE enabled = 1 ORDER BY RANDOM() LIMIT 1`,
        )
        .get() as EightBallRow | undefined;
      if (row) {
        database
          .prepare(
            `UPDATE eightball SET usage_count = usage_count + 1 WHERE id = ?`,
          )
          .run(row.id);
      }
      return row ?? null;
  ...
  countEnabled(): number {
    return (
      stmt(`SELECT COUNT(*) AS n FROM eightball WHERE enabled = 1`).get() as {
        n: number;
      }
    ).n;
  },
  ...
  countAll(): number {
    return (stmt(`SELECT COUNT(*) AS n FROM eightball`).get() as { n: number })
      .n;
  },

  list(limit = 25, offset = 0): EightBallRow[] {
    return stmt(
      `SELECT * FROM eightball ORDER BY id DESC LIMIT ? OFFSET ?`,
    ).all(limit, offset) as EightBallRow[];
  },

  get(id: number): EightBallRow | null {
    return (
      (stmt(`SELECT * FROM eightball WHERE id = ?`).get(id) as
        EightBallRow | undefined) ?? null
    );
  },

  remove(id: number): boolean {
    return stmt(`DELETE FROM eightball WHERE id = ?`).run(id).changes > 0;
  },

  edit(id: number, content: string): boolean {
    return (
      stmt(
        `UPDATE eightball SET content = ?, updated_at = datetime('now') WHERE id = ?`,
  ...
  setEnabled(id: number, enabled: boolean): boolean {
    return (
      stmt(
        `UPDATE eightball SET enabled = ?, updated_at = datetime('now') WHERE id = ?`,
