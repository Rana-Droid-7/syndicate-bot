...
/**
 * The single graceful-exit path, shared by /boot's DM panel and the
 * local dashboard. Ordering is load-bearing and must never drift
 * between surfaces: reminder timers stand down, the log sink drains
 * while the connection is alive, Discord disconnects, and the DB
 * closes LAST so in-flight deliveries can't race the close.
 *
