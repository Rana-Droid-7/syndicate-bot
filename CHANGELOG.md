...
- **Counts**: 40 unit + 44 integration/attack + 76 embed + 25 lookup + 27 doc + 3 timer checks + dispatcher torture — plus relation scans (alias collisions, usage-vs-alias consistency, example dispatch, suggestion coverage, slash deployment parity, tag-mirroring coverage) run during the audit.
...
### Cycle 8 — live cooldowns, statement caching, double-boot impossible
- **Cooldown errors are now LIVE countdowns** (the feature request that started this cycle): instead of a static "try again in **4s**" that was stale the moment it rendered, a cooldown hit replies with an embed carrying a Discord dynamic timestamp (re-rendered second-by-second by Discord itself in every client) PLUS a once-per-second edit loop with a shrinking progress bar (`▰▰▱▱ 2.4s / 5s`), flipping to a "ready" state exactly at expiry — on BOTH the prefix and slash lanes. The remaining time is queried at render time (`cooldowns.getRemaining()`), never baked at throw time. Edit failures (deleted message, lost perms) freeze the bar but the `<t:R>` tag keeps ticking.
- **Prepared-statement caching** across every repository: SQL was being re-parsed on every single call — every message's AFK check, every random joke, every warning add. A memoized `stmt()` helper compiles each query once per process; the cache is invalidated wholesale by `closeDb()` so a reopened connection never reuses a dead statement. This is the bot's real hot-path optimization: the per-message `isAfk` lookup, `forMentions`, joke/8-ball pulls, and warning lists all ride it.
- **Single-instance lock** (`data/bot.lock`): the audit sessions' double-instance incident — two live gateways doubling every announcement and mirror line — is now structurally impossible. A second boot checks the lockfile's PID against the live process table and refuses with a clear message; stale locks (crashed/killed PIDs) are reclaimed silently; the lock releases on every graceful exit path (signals, /boot shutdown, crash cleanup). Filesystem failures degrade to a loud warning, never a blocked boot.
